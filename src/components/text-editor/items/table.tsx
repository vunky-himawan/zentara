...
import { Button, Form, InputNumber, Modal, Popover, Space, Typography } from "antd";
...
const TableCell: React.FC<TableCellProps> = React.memo(
	({ isActive, onClick, onMouseEnter, onMouseLeave }) => {
...
const TableGrid: React.FC<TableGridProps> = ({ onInsertTable, maxRows = 6, maxCols = 6 }) => {
	...
	const displayText =
		hover.rows && hover.cols ? `${hover.rows} × ${hover.cols} table` : "Select table size";
...
const TableInsert: React.FC<TableInsertProps> = ({ editor, maxRows = 6, maxCols = 6 }) => {
	...
	return (
		<Popover
			content={<TableGrid onInsertTable={handleInsertTable} maxRows={maxRows} maxCols={maxCols} />}
			trigger={["click"]}
			placement="bottom"
			open={popoverVisible}
			...
			arrow={false}
		>
			<Button icon={<LucideIcon Icon={Table} />} type="text" size="small" />
		</Popover>
	);
};
